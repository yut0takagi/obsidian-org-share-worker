...
describe('GET /n/:uuid (private — no auth yet, just structure)', () => {
  it('returns 401 without CF-Access-Jwt-Assertion header', async () => {
    ...
    const app = new Hono()
    app.route('/', noteRoute)
    const res = await app.request('/n/priv1', {}, { R2: r2, ORG_DOMAIN: 'example.com', CF_ACCESS_TEAM_DOMAIN: 't.cloudflareaccess.com', CF_ACCESS_AUD: 'aud' } as any)
  ...
  it('returns 404 if missing', async () => {
    ...
    const app = new Hono()
    app.route('/', noteRoute)
    const res = await app.request('/n/missing', {}, { R2: r2, ORG_DOMAIN: 'example.com', CF_ACCESS_TEAM_DOMAIN: 't.cloudflareaccess.com', CF_ACCESS_AUD: 'aud' } as any)
    expect(res.status).toBe(404)
  })
})
